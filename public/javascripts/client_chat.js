...
socket.on('liste_utilisateurs_connectes', function(message) {
    ...
    for (let i = 0; i < liste_amis.length; i++){
        ...
        (function() {
            ...
            if(message.liste.indexOf(liste_amis[currentI]) != -1){
            ...
            var ami_a_modifier_apres_traitement = document.getElementById(liste_amis[currentI]);
           
            ami_a_modifier_apres_traitement.addEventListener("click", function() {
...
//Requete XHR periodique pour tester la connexion internet. Si on a perdu la connexion, affichage d'un message de deconnexion et deconnexion de la room
...
function test_connexion (){
    ...
    var xhr_test_connexion = new XMLHttpRequest();

    xhr_test_connexion.open('POST', '/test_connexion/');
    xhr_test_connexion.setRequestHeader('cache-control', 'no-cache, must-revalidate, post-check=0, pre-check=0');
    //xhr_test_connexion.setRequestHeader('cache-control', 'max-age=0');
   // xhr_test_connexion.setRequestHeader('expires', '0');
   // xhr_test_connexion.setRequestHeader('expires', 'Tue, 01 Jan 1980 1:00:00 GMT');
    xhr_test_connexion.setRequestHeader('pragma', 'no-cache');
    xhr_test_connexion.timeout = 1000;
    xhr_test_connexion.send(null);


    xhr_test_connexion.ontimeout = function (e) {
        console.log('TIMEOUT');
        
        xhr_test_connexion.abort();
    ...
    //Une fois que la requete est terminee - callback
    xhr_test_connexion.addEventListener('readystatechange', function() {
        if (xhr_test_connexion.readyState === 4 && (xhr_test_connexion.status === 200)) {
            ...
            //On demande de re-executer la fonction dans 20 sec
            /*
...
function creation_div_avec_valeur (nom_classe_div, id_div_to_append, valeur, facultatif_href_id, facultatif_non_cliquable){
    ...
    if(!facultatif_href_id){
        ...
        p_valeur.className = 'p_ami_affiche_deconnecte';
        p_valeur.id = valeur;
        //Append du texte dans le div
